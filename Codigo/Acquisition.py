...
def acquire_images(cam, nodemap, nodemap_tldevice, identifier):
    """
        ...
        for i in range(NUM_IMAGES):
            try:
                ...
                if image_result.IsIncomplete():
					...
					#Pyspin.PixelFormat_Mono8 era el original e imprimia la imagen en blanco y negro

                    # Create a unique filename
                    if device_serial_number:
                        filename = identifier + '.jpg' #'Acquisition-%s-%d.jpg' % (device_serial_number, i)
                    else:  # if serial number is empty
                        filename = identifier + '.jpg' #'Acquisition-%d.jpg' % i
...
def run_single_camera(cam, identifier):
    """
    ...
    try:
        ...
        # Acquire images
        result &= acquire_images(cam, nodemap, nodemap_tldevice, identifier)
...
def main():
    ...
    time.sleep(2)
    input('Compile and upload Arduino code into the board, press enter when completed')
    # Run example on each camera
    # Aca hay que coordinar cuantas se quieren sacar de acuerdo a lo que se ponga en el otro codigo.
    num_of_images = 25
    for im in range(num_of_images):
        ...
        for i, cam in enumerate(cam_list):
            ...
            result &= run_single_camera(cam,'Foto' + str(im))
